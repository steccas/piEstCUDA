...
#include "WarpCorrelated.cuh" //Libreria MWC64X, ci sono tre implementazioni diverse del warp
...
__global__ void mc_warp(int *hits, unsigned *state, int c)
{
    ...
    WarpCorrelated_LoadState(state, rngRegs, rngShmem);
    ...
    for(int j=0; j<c; j++){
        unsigned long x = WarpCorrelated_Generate(rngRegs, rngShmem);
        unsigned long y = WarpCorrelated_Generate(rngRegs, rngShmem);
    ...
    WarpCorrelated_SaveState(rngRegs, rngShmem, state);
...
int main(int argc, char **argv)
{
    ...
    if (t == 1)
    {
	    ...
	    */
        ...
        void *seedDevice = 0;
        unsigned totalRngs = totalThreads / WarpCorrelated_K;
        unsigned rngsPerBlock = BLOCK_SIZE / WarpCorrelated_K;
        unsigned sharedMemBytesPerBlock = rngsPerBlock * WarpCorrelated_K * 4;
        unsigned seedBytes = totalRngs * 4 * WarpCorrelated_STATE_WORDS;
